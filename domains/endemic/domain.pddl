(:(define (domain endemic)
    ...
    (:action fly_by_shuttle
        :parameters (?p - player ?c1 - city ?c2 - city)
        :precondition (and (in_city ?p ?c1) (has_research_station ?p ?c1) (has_research_station ?p ?c2)) 
        :effect (and (not (in_city ?p ?c1))  (in_city ?p ?c2))
    )   
    ...
    (:action build_research_station_new
        ...
        :effect (and (has_research_station ?p ?c1) (increase (research_station_count) 1))
    )   
    (:action move_research_station
        :parameters (?p - player ?c1 - city ?c2 - city)
        :precondition (and (in_city ?p ?c1) (has_research_station ?p ?c2) ) 
        :effect (and (has_research_station ?p ?c1) (not (has_research_station ?p ?c2)))
    )
    
    (:action treat_disease
        :parameters (?p - player ?c1 - city ?d - disease)
        :precondition (and (in_city ?p ?c1) (> (city_disease_count ?c ?d) 0) ) 
        :effect (and (decrease (city_disease_count ?c ?d) 1) (decrease (total_city_count ?c) 1) (decrease (total_disease_count ?c) 1))
    )

    (:action treat_cured_disease_begin
        :parameters (?p - player ?c1 - city ?d - disease)
        :precondition (and (in_city ?p ?c1) (> (city_disease_count ?c ?d) 0)  (is_cured ?d)) 
        :effect (and (cured_disease_began ?c ?d) (decrease (total_per_city_count ?c) (city_disease_count ?c ?d)) (decrease (total_disease_count ?c) (city_disease_count ?c ?d)))
    )

    (:action treat_cured_disease_end
        :parameters (?p - player ?c1 - city ?d - disease)
        :precondition (and (in_city ?p ?c1) (cured_disease_began ?c ?d)) 
